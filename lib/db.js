...
function initSchema() {
  ...
  // Admin wallet transactions (tracks revenue based on items assigned at account creation)
  database.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_wallet_created_at ON admin_wallet_transactions (created_at);
    CREATE INDEX IF NOT EXISTS idx_wallet_admin ON admin_wallet_transactions (admin_username);
  `);

  // Add optional columns for richer transaction history (safe migrations).
  try {
    database.exec(`ALTER TABLE admin_wallet_transactions ADD COLUMN kind TEXT`);
  } catch {}
  try {
    database.exec(`ALTER TABLE admin_wallet_transactions ADD COLUMN meta_json TEXT`);
  } catch {}
}
